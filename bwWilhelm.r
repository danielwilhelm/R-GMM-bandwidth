  ...
  weights = NULL, prewhite = 1, ar.method = "ols", data = list(), ...) 
{
  ...
  umat <- x$gt
  if(zoo::is.zoo(umat)) umat <- as.matrix(zoo::coredata(umat))
  n <- nrow(umat)
  k <- ncol(umat)
  ...
  if (p==k) return(bwAndrews(x, order.by=order.by, kernel=kernel, approx=approx, weights=weights, prewhite=prewhite, ar.method=ar.method, data=data, ...))
